...
object obstacleFactory inherits Factory { //otra opción: hacer una factory x obstáculo y tratarlo como las botellas
	...
	method buildObstacle() = //new Wall(position=self.random())
		if (nivelActual.es() == 1) new Wall(position=self.random())
		else if (nivelActual.es() == 2) new Fence(sufijo=suf2.anyOne(),position=self.random())
		else if (nivelActual.es() == 3) new Stump(sufijo=suf3.anyOne(),position=self.random())
		else 0
}
